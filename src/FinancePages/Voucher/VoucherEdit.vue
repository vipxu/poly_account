<template>
    <div id="VoucherEdit" v-voucherKD>
        <el-row class="el-card--custom">
            <el-row>
                ...
                <div class="fr">
                    ...
                    <div class="fl">
                        <el-button size="small" type="primary" icon="caret-left" :disabled="previd === ''" @click="prevClick"></el-button>
                        <el-button size="small" type="primary" icon="caret-right" :disabled="nextid === ''" @click="nextClick"></el-button>
                    </div>
                </div>
                <!-- right end -->
            </el-row>
...
export default {
    ...
    created() {
        ...
        this.getNowVoucher();//查询凭证
        this.getPrevNextVoucher();//查询前后凭证
    },
    methods: {
        //模板下拉菜单点击
        modCommand(command) {
        ...
        //上一张凭证
        prevClick() {
            this.$store.state.voucherUid = this.previd;
            this.getNowVoucher();
            this.getPrevNextVoucher();
        },
        nextClick() {
            this.$store.state.voucherUid = this.nextid;
            this.getNowVoucher();
            this.getPrevNextVoucher();
        },
        //下一张凭证
        ...
        //获取凭证字列表
        getVoucherWordOptions(year, period, dsign) {
            let Vue = this;
            Vue.AxiosPost('getVoucherWordOptions', {
                ...
                if (e.code === '1') {
                    for (let i = 0, l = e.data.dsignList.length; i < l; i++) {
                        ...
                        if (dl.dsignUid === dsign.dsignUid) {
                            Vue.voucherWordOptions[i] = {
                                value: dl.dsignUid,
                                label: dl.sign,
                                noId: dsign.inoId
                            };
                            Vue.voucherWordValue = dsign.dsignUid;
                            Vue.voucherNumberValue = dsign.inoId;
                        } else {
                            Vue.voucherWordOptions[i] = {
                                value: dl.dsignUid,
                                label: dl.sign,
        ...
        saveVoucher(accvouchs, add) {
            let Vue = this,
                data = {
                    ...
                    vouchUid: Vue.$store.state.voucherUid
                };
        ...
        getNowVoucher() {
            let Vue = this;
            Vue.AxiosPost('getNowVoucher', {
                vouchUid: Vue.$store.state.voucherUid
            }, function(e) {
                console.log(e);
                if (e.code === '1') {
                    Vue.getVoucherWordOptions(e.data.accvouch.iyear, e.data.accvouch.iperiod, { dsignUid: e.data.accvouch.dsignUid, inoId: e.data.accvouch.inoId });
                    Vue.voucherHeadInfo.aperiodUid = e.data.accvouch.periodUid;
                    Vue.voucherHeadInfo.iyear = e.data.accvouch.iyear;
                    Vue.voucherHeadInfo.iperiod = e.data.accvouch.iperiod;
                    Vue.billNum = e.data.accvouch.idoccount;
                    Vue.producer = e.data.accvouch.accbill;
                    Vue.pickerOptionsValue = e.data.accvouch.dbillDate;
                    Vue.pubts = Vue.timestampToDate(e.data.accvouch.pubts);
                    Vue.timeref = Vue.timestampToDate(e.data.accvouch.timeref);
                    Vue.$store.state.voucherUid = e.data.accvouch.vouchUid;
                    Vue.childData.trData = [];
                    Vue.$nextTick(() => {
                        e.data.accvouch.voucherRows.forEach((n, i) => {
                            Vue.childData.trData.push(n.vouchView);
                            Vue.getBalance(n.vouchView);
                            for (let j = 0, l = Vue.$store.state.subjectList.length; j < l; j++) {
                                //遍历科目列表
                                let n1 = Vue.$store.state.subjectList[j];
                                if (n1.id === Vue.childData.trData[i].subject.id) {
                    ...
                    if (e.data.accvouch.isCheckout === 1) {
                        //已结账
                        Vue.isCheckout = true;
                    } else {
                        //未结账
                        Vue.isCheckout = false;
                    }
                } else {
                    Vue.Message(e.msg, 'error');
                }
        ...
        getPrevNextVoucher() {
            let Vue = this;
            Vue.AxiosPost('getPrevNextVoucher', {
                presentVoucherUid: Vue.$store.state.voucherUid
            }, function(e) {
                if (e.code === '1') {
                    Vue.previd = e.data.arouchVoucherUid.prefixUid === '' ? '' : e.data.arouchVoucherUid.prefixUid;
                    Vue.nextid = e.data.arouchVoucherUid.suffixUid === '' ? '' : e.data.arouchVoucherUid.suffixUid;
                } else {
                    Vue.Message(e.msg, 'error');
                }
            });
        ...
        timestampToDate(timestamp) {
            ...
            return date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + ' ' + this.countnum(date.getHours() + '') + ':' + this.countnum(date.getMinutes() + '') + ':' + this.countnum(date.getSeconds() + '');
        },
        countnum(num) {
            let l = num.length;
            switch (l) {
    ...
    watch: {
        "$route": function(to, from) {
            if (from.params.page === '3-1' && to.params.page === '1-3') {
                this.vouchSaveId = this.randomString(10);
                this.getNowVoucher();
            }
        }
